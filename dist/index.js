...
var consultationPosts = mysqlTable("consultationPosts", {
  id: int("id").autoincrement().primaryKey(),
...
var consultationAttachments = mysqlTable("consultationAttachments", {
  id: int("id").autoincrement().primaryKey(),
...
var consultationReplies = mysqlTable("consultationReplies", {
  id: int("id").autoincrement().primaryKey(),
  postId: int("postId").notNull(),
...
async function createConsultationPost(input, attachments = []) {
...
async function listConsultationPosts({ query, page, pageSize }) {
  const db = requireDatabase(await getDb());
  const whereClause = query ? like(consultationPosts.title, `%${query}%`) : void 0;
  const selectItems = db.select({ id: consultationPosts.id, title: consultationPosts.title, status: consultationPosts.status, createdAt: consultationPosts.createdAt, views: consultationPosts.views }).from(consultationPosts).where(whereClause).orderBy(desc(consultationPosts.id)).limit(pageSize).offset((page - 1) * pageSize);
  const selectCount = db.select({ total: sql`count(*)` }).from(consultationPosts).where(whereClause);
  const [items, countRows] = await Promise.all([selectItems, selectCount]);
  return { items, total: Number(countRows[0]?.total ?? 0) };
}
async function getConsultationPost(id) {
  const db = requireDatabase(await getDb());
  const result = await db.select().from(consultationPosts).where(eq(consultationPosts.id, id)).limit(1);
  return result[0];
}
async function incrementConsultationPostViews(id) {
  const db = requireDatabase(await getDb());
  await db.update(consultationPosts).set({ views: sql`${consultationPosts.views} + 1` }).where(eq(consultationPosts.id, id));
}
async function listConsultationAttachments(postId) {
  const db = requireDatabase(await getDb());
...
async function getConsultationReply(postId) {
  const db = requireDatabase(await getDb());
  const result = await db.select().from(consultationReplies).where(eq(consultationReplies.postId, postId)).limit(1);
  return result[0];
}
async function upsertConsultationReply({ postId, body, adminUserId }) {
  const db = requireDatabase(await getDb());
  await db.transaction(async (transaction) => {
    await transaction.insert(consultationReplies).values({ postId, body, adminUserId }).onDuplicateKeyUpdate({ set: { body, adminUserId, updatedAt: /* @__PURE__ */ new Date() } });
...
// server/consultationSecurity.ts
import { randomBytes, scryptSync, timingSafeEqual } from "node:crypto";
function hashConsultationPassword(password) {
  const salt = randomBytes(16).toString("hex");
  const derived = scryptSync(password, salt, 64).toString("hex");
  return `${salt}:${derived}`;
}
function verifyConsultationPassword(password, encodedHash) {
...
// server/routers.ts
var MAX_ATTACHMENT_COUNT = 3;
var MAX_ATTACHMENT_BYTES = 2 * 1024 * 1024;
var ALLOWED_ATTACHMENT_TYPES = ["image/jpeg", "image/png", "application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"];
var consultationAttachmentInput = z2.object({
  fileName: z2.string().trim().min(1).max(255),
  mimeType: z2.enum(ALLOWED_ATTACHMENT_TYPES),
  contentBase64: z2.string().min(1).max(Math.ceil(MAX_ATTACHMENT_BYTES * 1.38))
});
...
var appRouter = router({
  ...
  consultation: router({
    create: publicProcedure.input(z2.object({
      title: z2.string().trim().min(2, "\uC0C1\uB2F4 \uC81C\uBAA9\uC744 2\uC790 \uC774\uC0C1 \uC785\uB825\uD574 \uC8FC\uC138\uC694.").max(160),
      applicantName: z2.string().trim().min(2, "\uC131\uD568\uC744 2\uC790 \uC774\uC0C1 \uC785\uB825\uD574 \uC8FC\uC138\uC694.").max(80),
      phone: z2.string().trim().min(9, "\uC5F0\uB77D\uCC98\uB97C \uD655\uC778\uD574 \uC8FC\uC138\uC694.").max(32),
      location: z2.string().trim().min(2, "\uD604\uC7A5 \uC704\uCE58\uB97C \uC785\uB825\uD574 \uC8FC\uC138\uC694.").max(255),
      workType: z2.string().trim().min(1, "\uACF5\uC815 \uC720\uD615\uC744 \uC120\uD0DD\uD574 \uC8FC\uC138\uC694.").max(80),
      schedule: z2.string().trim().min(1, "\uD76C\uB9DD \uC2DC\uAE30\uB97C \uC120\uD0DD\uD574 \uC8FC\uC138\uC694.").max(80),
      message: z2.string().trim().min(5, "\uBB38\uC758 \uB0B4\uC6A9\uC744 5\uC790 \uC774\uC0C1 \uC785\uB825\uD574 \uC8FC\uC138\uC694.").max(5e3),
      password: z2.string().min(4, "\uBE44\uBC00\uBC88\uD638\uB97C 4\uC790 \uC774\uC0C1 \uC785\uB825\uD574 \uC8FC\uC138\uC694.").max(64),
      attachments: z2.array(consultationAttachmentInput).max(MAX_ATTACHMENT_COUNT, `\uCCA8\uBD80\uD30C\uC77C\uC740 \uCD5C\uB300 ${MAX_ATTACHMENT_COUNT}\uAC1C\uAE4C\uC9C0 \uB4F1\uB85D\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4.`).default([])
    })).mutation(async ({ input }) => {
      const uploadedAttachments = await Promise.all(input.attachments.map(async (attachment) => {
        ...
        if (!data.length || data.length > MAX_ATTACHMENT_BYTES) throw new TRPCError3({ code: "BAD_REQUEST", message: "\uCCA8\uBD80\uD30C\uC77C\uC740 \uAC1C\uB2F9 2MB \uC774\uD558\uB85C \uB4F1\uB85D\uD574 \uC8FC\uC138\uC694." });
    ...
    list: publicProcedure.input(z2.object({ page: z2.number().int().min(1).default(1), query: z2.string().trim().max(80).default("") })).query(async ({ input }) => {
      const pageSize = 10;
    ...
    verifyDetail: publicProcedure.input(z2.object({ id: z2.number().int().positive(), password: z2.string().min(1) })).mutation(async ({ input }) => {
      const post = await getConsultationPost(input.id);
      if (!post || !verifyConsultationPassword(input.password, post.passwordHash)) {
        throw new TRPCError3({ code: "FORBIDDEN", message: "\uBE44\uBC00\uBC88\uD638\uAC00 \uC77C\uCE58\uD558\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4." });
      }
    ...
    upsertReply: adminProcedure.input(z2.object({ postId: z2.number().int().positive(), body: z2.string().trim().min(2, "\uB2F5\uBCC0\uC744 2\uC790 \uC774\uC0C1 \uC785\uB825\uD574 \uC8FC\uC138\uC694.").max(5e3) })).mutation(async ({ ctx, input }) => {
      const post = await getConsultationPost(input.postId);
      if (!post) throw new TRPCError3({ code: "NOT_FOUND", message: "\uC0C1\uB2F4 \uAC8C\uC2DC\uAE00\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
      const reply = await upsertConsultationReply({ postId: input.postId, body: input.body, adminUserId: ctx.user.id });
      return { status: "answered", reply: reply ? { body: reply.body, createdAt: reply.createdAt, updatedAt: reply.updatedAt } : null };
    })
  })
});
